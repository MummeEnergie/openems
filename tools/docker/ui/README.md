...
- OpenEMS Edge must be running on port 8075 (WebSocket)
...
- `WEBSOCKET_PORT`: Port for the Edge WebSocket server (default: `8075`)
...
Browser → UI (nginx:4200) → Edge (host.docker.internal:8075) → WebSocket
```
