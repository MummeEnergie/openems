...
if docker ps --format '{{.Names}}' | grep -q '^openems_ui_dev$'; then
    ...
    echo "   WebSocket:    host.docker.internal:8075 → $HOST_IP:8075"


    # Test WebSocket connectivity
    echo "Testing WebSocket connectivity..."
    if docker exec openems_ui_dev curl -s -o /dev/null -w "%{http_code}" http://host.docker.internal:8075 2>/dev/null | grep -q "404"; then
        echo "✅ WebSocket connection successful (Edge is reachable)"
    else
        echo "⚠️  Warning: Could not connect to Edge WebSocket"
        echo "   Make sure OpenEMS Edge WebSocket is running on port 8075"
    fi
else
